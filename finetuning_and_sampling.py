# -*- coding: utf-8 -*-
"""
Created on Tue Jun 28 00:08:14 2022
...
..author:: Max Pordon, TU Eindhoven, the Netherlands.
..date:: September 2017
...
def _save_flags_noargparse(filename):
    """ Function to save used arguments to log-file

    :return: saved file
    """
    with open(filename, 'w') as f:
        f.write("Used hyperparams:\n-----------\n")
        for hyperparam in hyperparams:
            f.write(str(hyperparam))
            f.write('\n')
...
def load_model_instance(filename):
    ...
    print(f'\n\n\nin load_model_instance, sample = {sample}\n\n\n')
    return mod
...
def save_model_instance(mod):
    ...
    """
    tmp = mod.model
    # print('tmp.save in', mod.checkpointdir, 'model.hdf5')
    # tmp.save(mod.checkpointdir + 'model.hdf5')  # ./finetune_and_sample_100_epoch_nr_1/checkpoint/model.hdf5 -- dit is in content/ -- moet zijn in drive/MyDrive/Q4_BEP/
    tmp.save(f'/content/drive/MyDrive/Q4_BEP/finetune_and_sample_{sample}_epoch_nr_{epoch}/checkpoint/model.hdf5')
    mod.model = None
    # print('save_model_instance save path:', mod.checkpointdir+'model.p\n')  # ./finetune_and_sample_100_epoch_nr_1/checkpoint/model.p -- dit is in content/ -- moet zijn in drive/MyDrive/Q4_BEP/
    # pickle.dump(mod, open(mod.checkpointdir + 'model.p', 'wb'))
    pickle.dump(mod, open(f'/content/drive/MyDrive/Q4_BEP/finetune_and_sample_{sample}_epoch_nr_{epoch}/checkpoint/model.p', 'wb'))
    mod.model = tmp
...
class Model(object):
    ...
    def __init__(self, n_vocab, outshape, session_name, cell="LSTM", n_units=256, batch=64, layers=2, lr=0.001,
                 dropoutfract=0.1, loss='categorical_crossentropy', l2_reg=None, ask=False, seed=42):
        ...
        if ask and os.path.exists(self.logdir):
            decision = input(f'\nSession folder already exists! ({self.logdir})\n'
                                 'Do you want to overwrite the previous session? [y/n] ')
            if decision in ['n', 'no', 'N', 'NO', 'No']:
                self.logdir = 'content/drive/MyDrive/Q4_BEP//' + input('Enter new session name: ')
    ...
    def finetuneinit(self, session_name, ask=True):
        ...
        """
        self.session_name = session_name
        self.logdir = 'content/drive/MyDrive/Q4_BEP//' + session_name
        if ask and os.path.exists(self.logdir):
            decision = input('\nSession folder already exists! (' + self.logdir + ')\n'
                                 'Do you want to overwrite the previous session? [y/n] ')
            if decision in ['n', 'no', 'N', 'NO', 'No']:
    ...
    def train(self, x, y, epochs=100, valsplit=0.2, sample=1000):
        ...
        writer = tf.summary.create_file_writer(f'/content/drive/MyDrive/Q4_BEP//' + self.session_name)
        with writer.as_default():
            for e in range(epochs):
                print("Epoch %i" % e)
                # checkpoints = [ModelCheckpoint(filepath=self.checkpointdir + 'model_epoch_%i.hdf5' % e, verbose=0)]
                print(f'\n\n\nin train, sample = {sample}\n\n\n')
                checkpoints = [ModelCheckpoint(filepath=f'/content/drive/MyDrive/Q4_BEP//' + self.session_name + '/checkpoint/model_epoch_0.hdf5', verbose=0)]
                ...
                # Empty the losses, because these are from pre-training. 
                # Keeping them raises an error with plotting val_losses.
                if epoch == 1:
                    print('First epoch --> deleting old losses....')
                    self.losses = []
    ...
    def plot_losses(self, show=False, cv=False):
        ...
        if cv:
            ...
            filename = f'/content/drive/MyDrive/Q4_BEP/finetune_and_sample_{sample}_epoch_nr_{epoch}' + '_cv_loss_plot.pdf'
            ...
            filename = f'/content/drive/MyDrive/Q4_BEP/finetune_and_sample_{sample}_epoch_nr_{epoch}/finetune_and_sample_{sample}_epoch_nr_{epoch}' + '_loss_plot.pdf'
    ...
    def sample(self, num=1000, minlen=7, maxlen=50, start=None, temp=2.5, show=False):
...
def main(infile, sessname, modfile, neurons=256, layers=2, epochs=1, batchsize=128, 
         window=0, step=1, target='all', valsplit=0.2, sample=1000, aa='B', 
         temperature=1.25, cell="LSTM", dropout=0.2, train=False, 
         learningrate=0.01, samplelength=22, pad=0, l2_rate=None, 
         cv=None, finetune=True, references=True):
    ...
    if finetune:
        ...
        model.finetuneinit(sessname, ask=False)  # generate new session folders for finetuning run
        ...
        # for _ in range(10):
        #     print()
        # with open('model_losses.txt', 'w') as f:
        #     f.write(str(model.losses))
    ...
    data.save_generated(f'/content/drive/MyDrive/Q4_BEP/finetune_and_sample_{sample}_epoch_nr_{epoch}//', 
                        f'/content/drive/MyDrive/Q4_BEP/finetune_and_sample_{sample}_epoch_nr_{epoch}/sampled_sequences_temp{str(temperature)}.csv')



if __name__ == "__main__":
    # parameters
    start_epoch, epochs = 15, 100
    infile = '/content/drive/MyDrive/Q4_BEP/Finetuning_dataset_8_peptides.csv' 
    sample = 1000
    temperature = 1.25
    dropout = 0.2
    train = False
    finetune = True
    valsplit = 0
    learningrate = 0.005  # half the lr of the pre-training. should be 1/10 but time constraints...
    samplelength = 22
    cv = False
    references = False
    hyperparams = ['infile', infile, 'sample', sample, 
                   'temperature', temperature, 'dropout', dropout, 
                   'train', train, 'finetune', finetune, 'valsplit', valsplit, 
                   'learningrate', learningrate, 'samplelength', samplelength,
                   'cv', cv, 'references', references]

    # Prevent DeprecationWarnings from tf
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        warnings.warn("deprecated", DeprecationWarning)

    for epoch in range(start_epoch, epochs+1):
        # sessname = f"finetune_and_sample_{sample}_epoch_nr_{epoch}"
        # sessname = 'content/drive/MyDrive/Q4_BEP//' + session_name
        ...
        # =============================================================
        # FOR FIRST EPOCH, LOAD THE PRE-TRAINED MODEL I.E. THE NON-FT'd
        # =============================================================
        if epoch == 1:
            # run main code
            main(modfile="/content/drive/MyDrive/Q4_BEP/mueller167v2/checkpoint/model_epoch_158.hdf5",
                 infile='/content/drive/MyDrive/Q4_BEP/Finetuning_dataset_8_peptides.csv', 
                 sample=1000,
                 sessname=f"finetune_and_sample_{sample}_epoch_nr_{epoch}", 
                 samplelength=22, train=False, finetune=True, valsplit=0
                 )

        # ======================================================================
        # FOR EVERY EPOCH APART FROM THE FIRST, LOAD THE PRE-TRAINED, FINE-TUNED 
        # MODEL 
        # ======================================================================
        else:
            # run main code
            main(
                modfile=f"/content/drive/MyDrive/Q4_BEP/finetune_and_sample_{sample}_epoch_nr_{epoch-1}/checkpoint/model_epoch_0.hdf5",
                #  modfile=f'/content/finetune_and_sample_100_epoch_nr_{epoch-1}/checkpoint/model_epoch_0.hdf5',
                 infile='/content/drive/MyDrive/Q4_BEP/Finetuning_dataset_8_peptides.csv',
                 sample=1000,
                 sessname=f"finetune_and_sample_{sample}_epoch_nr_{epoch}", 
                 samplelength=22, train=False, finetune=True, valsplit=0
            ...
            _save_flags_noargparse(f"/content/drive/MyDrive/Q4_BEP/finetune_and_sample_{sample}_epoch_nr_{epoch}" + "/flags.txt")
